...
This repository is automatically maintained — GitHub Actions checks the star count hourly and
regenerates the data and charts only when it has actually changed.
...
- [`.github/workflows/update-star-history.yml`](.github/workflows/update-star-history.yml)
  runs hourly (at :23) and commits only when the data changed. When the star count matches
  what is already stored, the script writes nothing and the run ends without a commit, so an
  unchanged hour — or an unchanged month — leaves no trace in the history.
